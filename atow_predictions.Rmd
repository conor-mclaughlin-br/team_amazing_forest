...
```{r}
saveRDS(rf_model, file = "saved_models/random_forest_model_v18.rds")
```
...
```{r}
# Assuming rf_model is your ranger model
importance_data <- as.data.frame(rf_model$variable.importance)

# Convert the row names (variable names) into a column
...
```{r}
# Calculate residuals
residuals <- test_data$tow - predictions
```

```{r}
# Histogram of residuals
hist(residuals, main = "Histogram of Residuals", xlab = "Residuals", breaks = 50)
```
...
```{r}
# MAE and % Miss vs Actuals
test_data %>% 
  mutate(
    residual = tow - predictions,
    prediction = predictions
  ) %>% 
  group_by(aircraft_type) %>% 
  summarize(
    count = n(),
    pct_of_total = (n() / nrow(test_data)) * 100,
    mae = mean(abs(residual)),
    avg_tow = mean(tow),
    max_pct_tow = mean(abs(residual)) / mean(tow),
    mape = mean(abs(residual / tow)) * 100,
    rmse = mean(sqrt((residual)^2)),
...
test_data %>% 
  ...
  group_by(interaction_aircraft_type_cluster_airline) %>% 
  summarize(
    count = n(),
    pct_of_total = (n() / nrow(test_data)) * 100,
    mae = mean(abs(residual)),
...
test_data %>% 
  ...
  group_by(aircraft_type_cluster) %>% 
  summarize(
    count = n(),
    pct_of_total = (n() / nrow(test_data)) * 100,
    mae = mean(abs(residual)),
...
test_data %>% 
  ...
  group_by(flight_minus_taxiout_duration_bucket) %>% 
  summarize(
    count = n(),
    pct_of_total = (n() / nrow(test_data)) * 100,
    mae = mean(abs(residual)),
...
test_data %>% 
  ...
  group_by(great_circle_distance_bucket) %>% 
  summarize(
    count = n(),
    pct_of_total = (n() / nrow(test_data)) * 100,
    mae = mean(abs(residual)),
...
```{r}
# Assuming you have a factor variable like `aircraft_type`
boxplot(residuals ~ test_data$aircraft_type, 
        main = "Residuals by Aircraft Type", 
        xlab = "Aircraft Type", 
        ylab = "Residuals")
```

```{r}
# Assuming `time` is your time variable
...
# cor_matrix <- cor(final_summary %>% select_if(is.numeric), use = "complete.obs")
# 
# # Find highly correlated features (above a threshold, e.g., 0.9)
# high_cor_features <- findCorrelation(cor_matrix, cutoff = 0.9)
# 
# # Remove highly correlated features
# # reduced_summary <- final_summary %>% select(-all_of(high_cor_features))
